function [S, Sigma, obj] = GOAL_MVC(KH, option,lambda)
num = size(KH, 1);
numker = size(KH, 3);
%--------------------------------------------------------------------------------
...
A_gamma = sumKbeta(KH, exp(Sigma));
[S, obj1] = solve_S(A_gamma,lambda);
...
while loop
    ...
    [Sigma,S,obj(nloop)] = graphupdate(KH,Sigmaold,grad,obj(nloop-1),option,lambda);
